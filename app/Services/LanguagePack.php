...
class LanguagePack {
    ...
    function __construct($code = false) {
        ...
        if (!$_for_translate) {
            $this->_default = $_for_translate = Keyvalue::read(H . '/storagelanguages/for_translate.lng');
        } else {
    ...
    protected function _load() {
        ...
        $data = array();
        $lngs = (array) glob(H . '/storage/languages/' . $this->code . '/*.lng');
    ...
    protected function _addString($str) {
        ...
        Keyvalue::save(H . '/storage/languages/' . $this->code . '/' . $this->_script . '.lng', $this->_data[$this->_script]);
        $this->clearCache();
    }
    ...
    function __destruct() {
        if ($this->_save_default) {
            Keyvalue::save(H . '/storage/languages/for_translate.lng', $this->_default);
        }
    }
